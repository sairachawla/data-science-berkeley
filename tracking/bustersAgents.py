    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        positions = [ghostDistribution.argMax() for ghostDistribution in livingGhostPositionDistributions]

        
        action = None
        count = 0
        smallest = float("inf") #comparison

        for a in legal:
            lst = [self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, a), p) for p in positions]
            d = min(lst)
            if d <= smallest:
                action = a
                smallest = d
