...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        sub_first = scheme_eval(first, env)
        if check_procedure(sub_first): 
            raise SchemeError('Not a scheme procedure')
        else:
            return scheme_apply(sub_first, rest.map(lambda x: scheme_eval(x,env)), env)
        # END PROBLEM 5
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if formals is not nil:
            raise SchemeError('too many formal parameters')  
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        evaluated_value = scheme_eval(expressions.second.first, env)
        env.define(target, evaluated_value)
        return target
        # END PROBLEM 6
        ...
        first = target.first
        rest = target.second
        evaluated_value = do_lambda_form(Pair(rest, expressions.second), env)
        env.define(first, evaluated_value)
        return first
        # END PROBLEM 10
    else:
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 9
    new_lambda = LambdaProcedure(formals, expressions.second, env)
    return new_lambda
    # END PROBLEM 9
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    f = bindings.map(lambda a: a.first)
    vals = bindings.map(lambda a: scheme_eval(a.second.first, env))
    ...
    return env.make_child_frame(f, vals)
    # END PROBLEM 15
